...
cat("Cosunt of Negative values:", sum(num_leaf_df[num_leaf_df<0]), "\n")
...
# plot(num_leaf_df)
# contour(x = num_leaf_df.mdens$x1, y = num_leaf_df.mdens$x2, z = num_leaf_df.mdens$fhat, add = TRUE)
...
ct <- cutree(hcs, #)
table(ct)
c1 <- subset(rownames(num_leaf_df), ct == #)
index1 <- match(c1, rownames(num_leaf_df))
```
...
leaf.km <- kmeans(num_leaf_df, centers = #)
...
plot.wgss(num_leaf_df, #)

plot(num_leaf_df.pca_cor$scores[,1:2], col = leaf.km$cluster)
