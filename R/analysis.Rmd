...
>>>>>>> a86cf21c0c275ba95829161a9eba73e0ddcbf7fb
...
remove_outliers(leaf_df, as.vector(colnames(leaf_df)))
...
```{r]}
techst <- hist(leaf_df$Lobedness)
leaf_df$Lobedness
plot(techst, main="Lobedness Histogram")
Elong <- hist(leaf_df$Elongation)
leaf_df$Elongation
...
```{r]}
plot(leaf_df$Elongation,leaf_df$Lobedness ,
...
ct <- cutree(hcs, #)
table(ct)
c1 <- subset(rownames(leaf_df), ct == #)
index1 <- match(c1, rownames(leaf_df))
```
...
leaf.km <- kmeans(leaf_df, centers = #)
...
plot(leaf_df.pca_cor$scores[,1:2], col = leaf.km$cluster)
